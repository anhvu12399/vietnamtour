...
const defaultPillars = [
  { title: 'Unique to You', desc: 'We listen to your travel goals and craft unique trips that are completely bespoke to you.' },
  { title: 'Seamless Service', desc: 'UK-based specialists available 24/7, providing end-to-end support throughout your journey.' },
  { title: 'Carefully Curated', desc: "We've hand-selected only the finest properties, private cruises, and experiences in Vietnam." },
  { title: 'Luxury in Every Sense', desc: "Awarded Condé Nast Traveller's Top Travel Specialists for exemplary bespoke service." },
];

const defaultGridCategories = [
  { title: 'A Lifetime of Memories', image: '/images/vietnamtour_halong_yacht_luxury.png', link: '/itineraries/the-grand-tour-of-vietnam' },
  { title: 'Elite Resort Offers', image: '/images/vietnamtour_amanoi_villa.png', link: '/accommodations' },
  { title: 'Best Winter Sun Spots', image: '/images/vietnamtour_phu_quoc_beach.png', link: '/destinations/phu-quoc-island' },
  { title: 'Top Culinary Expeditions', image: '/images/vietnamtour_cave_dining.png', link: '/itineraries/vietnamese-culinary-and-culture-journey' },
  { title: 'Bucket List Journeys', image: '/images/vietnamtour_sapa_lodge.png', link: '/itineraries' },
  { title: 'Active & Wilderness', image: '/images/vietnamtour_mekong_sampan.png', link: '/destinations/hanoi-and-the-north' },
];

const defaultExperiences = [
  { title: 'Cave Dining in Halong Bay', desc: 'A candle-lit gourmet banquet set up inside a private volcanic rock cave, listening to the gentle lap of waves.', image: '/images/vietnamtour_cave_dining.png', badge: 'Included in Luxury Programmes', link: '/enquire' },
  { title: 'Private Lan Ha Bay Catamaran', desc: 'Charter our custom catamaran to explore secret lagoons and limestone karsts far from the standard tourist paths.', image: '/images/vietnamtour_halong_yacht_luxury.png', badge: 'Available for Private Booking', link: '/enquire' },
  { title: 'Misty Sapa Sanctuary Retreat', desc: 'Wake up above the clouds in a hand-crafted cedar lodge overlooking emerald rice terraces, with a private local guide.', image: '/images/vietnamtour_sapa_lodge.png', badge: 'Featured in Adventure Tours', link: '/enquire' },
];

const defaultSteps = [
  { num: '1', title: 'Make an Enquiry', desc: 'Submit a holiday enquiry online or connect via phone.' },
  { num: '2', title: 'Speak to an Expert', desc: 'Discuss your travel plans with your dedicated travel specialist.' },
  { num: '3', title: 'Receive a Quote', desc: 'We craft your dream holiday draft and send you a detailed proposal.' },
  { num: '4', title: 'Book Your Trip', desc: 'Once every detail is just right, we confirm your booking and protection.' },
];

const defaultPressQuotes = [
  { quote: '"With different interests, ages, and pacing, Vietnam Tour ensures every adventure is packed with special moments and bespoke touches."', source: 'COUNTRY & TOWN HOUSE' },
  { quote: '"For nearly four decades of expertise, Vietnam Tour has been fashioning bespoke and tailormade holidays, working 24/7 to perfect every detail."', source: 'THE STANDARD' },
  { quote: '"...this global tour operator specialises in high-end holidays, curating the most spectacular retreats and local encounters for the UK market."', source: 'CONDÉ NAST TRAVELLER' },
];

const defaultReviews = [
  { title: 'Bespoke Perfection: Our 2-Week Luxury Journey', text: 'Vietnam Tour exceeded every expectation. Alice Mercer designed a flawless honeymoon covering Amanoi, a private catamaran in Lan Ha Bay, and historic sidecar tours in Hanoi. Every hotel stay included exclusive upgrades, and our private guides felt like family. Truly a bespoke luxury experience.', author: 'Charles & Victoria S.', location: 'London, UK', rating: 5, date: 'June 2026', highlights: ['Private Yacht Charter', 'Bespoke Itinerary', 'Amanoi Retreat'] },
  { title: 'Flawless Multi-Generational Family Holiday', text: 'Traveling with three generations is challenging, but Alice orchestrated a seamless flow. From private cooking classes inside rock caves to slow luxury sampan cruising in the Mekong Delta, everything was tailored to our pacing. The attention to detail was incredible.', author: 'The Sinclair Family', location: 'Surrey, UK', rating: 5, date: 'May 2026', highlights: ['Mekong Sampan', 'Cave Dining', 'Private Guides'] },
  { title: 'Unrivaled Local Access and Elite Service', text: 'James Harrison set us up with local historians in Saigon and arranged a private helicopter transfer to Ninh Van Bay. His insider tips on Ninh Thuan were spot-on. Vietnam Tour is in a league of its own for luxury Indochina travel. Worth every penny.', author: 'Sir Arthur & Lady Evelyn', location: 'Edinburgh, Scotland', rating: 5, date: 'April 2026', highlights: ['Helicopter Transfer', 'Expert Insights', 'VIP Airport Meet'] },
];
...
export default async function HomePage() {
  try {
    ...
    // Resolve data — Sanity first, fallbacks second
    const valuePillars = homepage?.valuePillars?.length ? homepage.valuePillars : defaultPillars;
    const gridCategories = homepage?.gridCategories?.length ? homepage.gridCategories : defaultGridCategories;
    const experiences = homepage?.experiences?.length ? homepage.experiences : defaultExperiences;
    const planningSteps = homepage?.steps?.length ? homepage.steps : defaultSteps;
    const pressQuotes = homepage?.pressQuotes?.length ? homepage.pressQuotes : defaultPressQuotes;
    const tripAdvisorReviews = homepage?.reviews?.length ? homepage.reviews : defaultReviews;
      ...
      <>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-16 items-start">
            <div className="space-y-6 lg:sticky lg:top-32">
              ...
              <div className="pt-4 flex flex-wrap gap-4">
                <Link
                  href={homepage?.introButtonLink || '/itineraries'}
                ...
                >
                  {homepage?.introButtonText || 'Discover Our Journeys'}
                </Link>
              </div>
            </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 items-center">
              <div className="space-y-6 max-w-xl">
                ...
                <div className="pt-4">
                  ...
                  >
                    {homepage?.artOfTravelButtonText || 'Meet Our Experts'}
                  </Link>
                </div>
              </div>
              ...
              <div className="relative h-[450px] border border-luxury-gold/20 overflow-hidden group">
                <Image
                  src={homepage?.artOfTravelImage || '/images/vietnamtour_sapa_lodge.png'}
                  alt={homepage?.artOfTravelImageTitle || 'Featured'}
                  fill
                  className="object-cover brightness-75 group-hover:scale-102 transition-transform duration-500"
                />
                <div className="absolute inset-0 bg-black/30 z-10" />
                <div className="absolute bottom-8 left-8 z-20">
                  {(homepage?.artOfTravelImageLabel) && (
                    <span className="text-xs uppercase tracking-[0.3em] font-semibold text-luxury-gold block">{homepage.artOfTravelImageLabel}</span>
                  )}
                  {(homepage?.artOfTravelImageTitle) && (
        ...
        <section className="py-24 max-w-7xl mx-auto px-6 lg:px-12 space-y-16">
          <div className="text-center space-y-3 max-w-2xl mx-auto">
            <span className="text-xs uppercase tracking-[0.3em] font-semibold text-luxury-gold">
              {homepage?.experiencesLabel || 'Bespoke Elements'}
            </span>
            <h2 className="font-serif text-3xl lg:text-4xl text-luxury-linen font-medium">
              {homepage?.experiencesHeading || 'Signature Luxury Experiences'}
            </h2>
            ...
            <p className="text-xs sm:text-sm text-luxury-linen/70 font-light leading-relaxed">
              {homepage?.experiencesDescription || 'Unique activities crafted exclusively for Vietnam Tour travelers, ensuring your holiday is unlike any other.'}
            </p>
          </div>
        ...
        <section className="py-24 max-w-7xl mx-auto px-6 lg:px-12 text-center space-y-16">
          <div className="space-y-3 max-w-2xl mx-auto">
            <h2 className="font-serif text-3xl lg:text-4xl text-luxury-linen font-medium">
              {homepage?.stepsHeading || 'Four Steps to Your Luxury Holiday'}
            </h2>
            <div className="h-[1px] w-20 bg-luxury-gold mx-auto mt-4" />
          </div>
          ...
          <div className="pt-6">
            ...
            >
              {homepage?.stepsButtonText || 'Design My Holiday'}
            </Link>
          </div>
        </section>
        ...
        <section className="relative overflow-hidden border-y border-luxury-gold/20">
          <div className="absolute inset-0 z-0">
            <Image
              src={homepage?.pressBackgroundImage || '/images/press_hero_colonial.png'}
          ...
          <div className="relative z-10 max-w-7xl mx-auto px-6 lg:px-12 py-28">
            <div className="text-center mb-16 space-y-3">
              <span className="text-[10px] uppercase tracking-[0.4em] font-semibold text-luxury-gold block">
                {homepage?.pressLabel || 'As Featured In'}
              </span>
              <h2 className="font-serif text-4xl lg:text-5xl text-white font-light">
                {homepage?.pressHeading ? (
                  homepage.pressHeading
                ) : (
                  <>In the<span className="italic text-luxury-gold font-medium"> Press</span></>
                )}
              </h2>
        ...
        <section className="relative bg-luxury-slate text-luxury-linen py-24 overflow-hidden border-t border-luxury-gold/20">
          ...
          <div className="relative z-10 max-w-4xl mx-auto px-6 text-center space-y-8">
            <span className="text-xs uppercase tracking-[0.3em] font-semibold text-luxury-gold">
              {homepage?.finalCtaLabel || 'Bespoke Planning'}
            </span>
            <h2 className="font-serif text-3xl sm:text-4xl lg:text-5xl font-medium leading-snug">
              {homepage?.finalCtaHeading || 'Draft Your Dream Itinerary'}
            </h2>
            <p className="text-sm sm:text-base font-light text-luxury-linen/70 max-w-2xl mx-auto leading-relaxed">
              {homepage?.finalCtaSubtext || 'Every holiday we design is unique. Speak directly to Alice Mercer or James Harrison to tailor your travel dates, properties, and dining pacing.'}
            </p>
            <div className="flex flex-col sm:flex-row items-center justify-center gap-6 pt-4">
              ...
              >
                {homepage?.finalCtaButtonText || 'Start Planning'}
              </Link>
              {homepage?.finalCtaPhone && (
                <a
                  href={`tel:${homepage.finalCtaPhone.replace(/\s/g, '')}`}
                  className="text-sm font-semibold tracking-wider text-luxury-gold hover:text-white transition-colors"
                >
                  Call {homepage.finalCtaPhone}
                </a>
              )}
              {!homepage?.finalCtaPhone && (
                <a
