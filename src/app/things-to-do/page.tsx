    ...
    <>
      ...
      <main className="min-h-screen bg-luxury-slate text-luxury-linen">
        ...
        <section className="py-16 px-6 lg:px-12 max-w-4xl mx-auto text-center">
          ...
          <h2 className="font-serif text-2xl sm:text-3xl text-luxury-linen font-medium leading-snug mb-5">
            Not a checklist. A collection of moments.
          </h2>
          <p className="text-base text-luxury-linen/75 font-light leading-relaxed max-w-2xl mx-auto">
        ...
        <section className="pb-24 px-6 lg:px-12 max-w-7xl mx-auto">
          ...
          <div className="mb-10">
            {(() => {
              ...
              const colorClass = categoryColors[featured.category] || 'bg-luxury-slate/30 text-luxury-gold border-luxury-gold/20';
              return (
                <Link href={`/things-to-do/${featured.slug}`} className="group block">
                  <div className="grid grid-cols-1 lg:grid-cols-2 gap-0 border border-luxury-gold/10 hover:border-luxury-gold/30 transition-all duration-500 hover:shadow-2xl overflow-hidden">
                    {/* Image */}
                    <div className="relative h-72 sm:h-80 lg:h-[420px] overflow-hidden bg-luxury-slate/20">
                      <Image
                    ...
                    {/* Content */}
                    <div className="p-8 sm:p-10 lg:p-14 flex flex-col justify-center bg-luxury-slate/30 space-y-5">
                      <div className="flex items-center gap-3">
                        <span className={`text-[9px] uppercase tracking-[0.25em] font-bold px-3 py-1 border rounded-sm ${colorClass}`}>
                          {featured.category}
                        </span>
                        <span className="text-[10px] text-luxury-linen/40 font-medium">{featured.readingTime} min read</span>
                      </div>

                      <h2 className="font-serif text-2xl sm:text-3xl lg:text-4xl text-luxury-linen font-medium leading-snug group-hover:text-luxury-gold transition-colors duration-300">
                        {featured.title}
                      </h2>

                      <p className="text-sm text-luxury-linen/70 font-light leading-relaxed line-clamp-3">
                        {featured.heroSubtitle}
                      </p>

                      <div className="flex items-center gap-2 pt-2">
                        <span className="text-[11px] uppercase tracking-widest font-bold text-luxury-gold group-hover:text-luxury-linen transition-colors">
                          Read the full guide
                        </span>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {thingsToDoData.slice(1).map((item) => {
              const colorClass = categoryColors[item.category] || 'bg-luxury-slate/30 text-luxury-gold border-luxury-gold/20';
              return (
                <Link
                  key={item.slug}
                  href={`/things-to-do/${item.slug}`}
                  className="group flex flex-col h-full border border-luxury-gold/10 hover:border-luxury-gold/30 transition-all duration-400 hover:shadow-xl overflow-hidden bg-luxury-slate/20"
                >
                  {/* Image */}
                  <div className="relative h-56 overflow-hidden bg-luxury-slate/20 flex-shrink-0">
                    <Image
                      src={item.heroImage}
                      alt={item.title}
                      fill
                  ...
                  <div className="p-6 flex flex-col flex-grow space-y-3">
                    <div className="flex items-center gap-2">
                      <span className="text-[9px] text-luxury-linen/40 font-medium uppercase tracking-widest">{item.readingTime} min read</span>
                      <span className="text-luxury-linen/20">·</span>
                      <span className="text-[9px] text-luxury-linen/40 font-medium uppercase tracking-widest">{item.practicalInfo.location.split(',')[0]}</span>
                    </div>

                    <h3 className="font-serif text-lg sm:text-xl text-luxury-linen font-medium leading-snug group-hover:text-luxury-gold transition-colors duration-300">
                      {item.title}
                    </h3>

                    <p className="text-xs text-luxury-linen/60 font-light leading-relaxed line-clamp-3 flex-grow">
                      {item.heroSubtitle}
                    </p>

                    <div className="pt-3 border-t border-luxury-gold/10 flex items-center justify-between">
                      <ul className="space-y-0.5">
                        {item.highlights.slice(0, 2).map((h, i) => (
                          <li key={i} className="text-[10px] text-luxury-linen/55 flex items-start gap-1.5 line-clamp-1">
                    ...
                    <div className="flex items-center gap-1.5 pt-1">
                      <span className="text-[10px] uppercase tracking-widest font-bold text-luxury-gold group-hover:text-luxury-linen transition-colors">
                        Read guide
                      </span>
        ...
        {/* ── Practical Info Bar ── */}
        <section className="border-t border-luxury-gold/10 bg-luxury-slate/30 py-14 px-6 lg:px-12">
          <div className="max-w-5xl mx-auto grid grid-cols-2 md:grid-cols-4 gap-8 text-center">
            {[
              ...
              <div key={i} className="space-y-2">
                ...
                <p className="text-xs text-luxury-linen/55 font-light leading-relaxed">{item.desc}</p>
              </div>
            ))}
          </div>
        </section>
        ...
        {/* ── Bottom CTA ── */}
        <section className="bg-[#161C1A] py-20 px-6 lg:px-12 relative overflow-hidden">
          ...
          <div className="max-w-3xl mx-auto text-center space-y-8 relative z-10">
            ...
            <p className="text-base text-luxury-linen/75 font-light max-w-xl mx-auto leading-relaxed">
            ...
            <div className="flex flex-col sm:flex-row items-center justify-center gap-4 pt-4">
              ...
              <Link
                href="/itineraries"
                className="w-full sm:w-auto border border-luxury-gold/30 hover:border-luxury-gold text-luxury-linen text-xs uppercase tracking-widest font-bold px-10 py-4 transition-colors duration-300"
              >
                View Vietnam Tours
              </Link>
            </div>
