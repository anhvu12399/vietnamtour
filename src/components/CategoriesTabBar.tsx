...
export default async function CategoriesTabBar({ activeTab }: CategoriesTabBarProps) {
  ...
  return (
    <section className="w-full bg-[#121816] text-white sticky top-[88px] z-30 shadow-sm border-b border-white/5">
          ...
          {tabs.map((tab) => {
            ...
            return (
              <Link
                key={tab.id}
                href={tab.href}
                className="relative h-full flex items-center gap-1.5 hover:text-[#c5a880] transition-colors duration-200 cursor-pointer"
              >
                <span
                ...
                {isActive && (
                  ...
                  <div className="absolute bottom-0 left-1/2 -translate-x-1/2 translate-y-full w-0 h-0 border-l-[6px] border-l-transparent border-r-[6px] border-r-transparent border-t-[6px] border-t-[#121816] z-10" />
                )}
              </Link>
            );
          })}
