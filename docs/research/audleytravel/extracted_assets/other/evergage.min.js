...
var audleySitemapConfigHelpers = {
    getPageTemplateFromDataLayer: () => {
        try {
            if (window.dataLayer) {
                for (let i = 0; i < window.dataLayer.length; i++) {
                    if (window.dataLayer[i].event == "pageMetaData") {
                        return window.dataLayer[i].page.template;
                    }
                }
            }
            return '';
    ...
    isPageType: (pp) => {
        try {
            let page = audleySitemapConfigHelpers.pageTypes.find(e => e.pageType === pp);
            if (page != null && page.dataLayerTemplateName != null) {
                return page.dataLayerTemplateName === audleySitemapConfigHelpers.getPageTemplateFromDataLayer();
            }
            if (page != null && page.matchUrl) {
                return page.matchUrl === window.location.host;
            }
            return false;
        } catch (error) {
            return false;
    ...
    getPageName: () => {
        try {
            if (window.dataLayer) {
                for (let i = 0; i < window.dataLayer.length; i++) {
                    if (window.dataLayer[i].event == "pageMetaData") {
                        return window.dataLayer[i].page.title;
                    }
                }
            }
            return '';
    ...
    getDestination: () => {
        try {
            if (window.dataLayer) {
                for (let i = 0; i < window.dataLayer.length; i++) {
                    if (window.dataLayer[i].event == "pageMetaData") {
                        return audleySitemapConfigHelpers.capitalizeFirstLetterOfEveryWord(window.dataLayer[i].page.category1.replaceAll("-", " "));
                    }
                }
            }
            return '';
    ...
    getInspirationCategory: () =>{
        try {
            if (window.AUDLEY)
                return AUDLEY.Context.InspirationCategory;
        } catch (error) {
            return null;
        }
    },
    getHolidayTypes: () => {
        try {
            if (window.AUDLEY)
                return AUDLEY.Itinerary.Categories;
        } catch (error) {
            return null;
        }
    },
    capitalizeFirstLetterOfEveryWord: (intput) => {
        try {
            if (!intput || intput.length === 0)
                return '';
            const words = intput.split(" ");
    ...
    getTourHighlights: () => {
        try {
            const tourHighlights = Evergage.cashDom('.tour-highlights ul');
            if (tourHighlights && tourHighlights.length != 0) {
                let highlights = tourHighlights[0].innerText.trim();
                let maxChar = 350;
                if (highlights.length <= maxChar) {
                    return highlights;
                }
                else {
                    const words = highlights.split(" ");
                    let newHighlights = "";
                    for (let i = 0; i < words.length; i++) {
                        let currentWord = words[i];
                        if ((newHighlights.length + currentWord.length) > maxChar)
                            break;
                        newHighlights = newHighlights + " " + currentWord;
                    }
    ...
    getMainImage: () => {
        try {
            // const imgs =  document.querySelectorAll('.hero-gallery__img picture img');
            // if(imgs.length>2)
            //     return document.querySelectorAll('.hero-gallery__img picture img')[1].src;

            // return document.querySelectorAll('.hero-gallery__img picture img')[0].src;
            return Evergage.cashDom('meta[property="og:image"]').attr('content');
        } catch (error) {
            return null;
        }
    },
    getPrice: () => {
        try {
            var priceItinerary = Evergage.cashDom('p[class*="hero__price"] strong')[0];
            if (priceItinerary) {
                return priceItinerary.innerText;
            }
            var priceHbItinerary = Evergage.cashDom('.cols p:nth-child(3) span')[0];
            if (priceHbItinerary) {
                return priceHbItinerary.innerText;
            }
        } catch (error) {
            return '';
        }
    ...
    getTripDuration: () => {
        try {
            const tripDurationItinerary = Evergage.cashDom('p[class*="hero__price"]');
            if (tripDurationItinerary && tripDurationItinerary.length > 0) {
                const days = tripDurationItinerary[0].innerText.match(/(.*)days/);
                if (days)
                    return parseInt(days[1].trim());
            }
            const tripDurationHItinerary = Evergage.cashDom('.cols p:nth-child(3)');
            if (tripDurationHItinerary && tripDurationHItinerary.length > 0) {
                const days = tripDurationHItinerary[0].innerText.match(/(.*)days/);
                if (days)
                    return parseInt(days[1].trim());
            }
        } catch (error) {
    ...
    getItemID: () => {
        try {
            if (window.AUDLEY)
                return AUDLEY.Context.Item.Id.replaceAll("{", "").replaceAll("}", "");
        } catch (error) {
            return null;
        }
    },
    getCurrencyCode: () => {
        try {
            if (window.AUDLEY)
                return AUDLEY.Visitor.CurrencyCode;
        } catch (error) {
            return null;
        }
    },
    getSfmcKeyFromURL: () => {
        try {
            ...
            // Extract the old subscriber key (ensuring no preceding underscore)
            const oldKeyMatch = url.match(/[^_]subscriberkey=(.*?)&|[^_]subscriberkey=(.*)/);
            // Extract the new tce subscriber key
            const newKeyMatch = url.match(/tce_subscriberkey=(.*?)&|tce_subscriberkey=(.*)/);
            
            return {
                // Safely finds the actual extracted value regardless of where it falls in the regex match groups
                oldKey: oldKeyMatch ? oldKeyMatch.slice(1).find(match => match !== undefined) : null,
                newKey: newKeyMatch ? newKeyMatch.slice(1).find(match => match !== undefined) : null
            };
        } catch (error) {
    ...
    getContactFormInputData: () => {
        try {
            var destination = Evergage.cashDom('#Destination').val();
            if(destination == null || destination == 'undefined' || destination.length == 0)
                destination = 'undecided';
            
            return {
                title: Evergage.cashDom('select#Title').val(),
                destination: destination,
                firstName: Evergage.cashDom('#FirstName').val(),
                lastName: Evergage.cashDom('#LastName').val(),
                email: Evergage.cashDom('#Email').val(),
                telephone: Evergage.cashDom('#Telephone').val(),
            }
        } catch (error) {
            return null;
        }
    ...
    getTourPlaces: () => {
        try {
            let tourMapData = Evergage.cashDom('#tour-map').attr('data-accommodation');
            let places = "";
            if (tourMapData) {
                let placeButtons = Evergage.cashDom('.itinerary__button--iconed');
                if (placeButtons) {
                    for (let i = 0; i < placeButtons.length; i++) {
                        let place = placeButtons[i].getAttribute('data-lightbox-trigger');
                        if (place != null && place.indexOf(":") === -1) {
                            let placeName = JSON.parse(tourMapData)[place].Name;
            ...
            return places.length > 0 ? places.substring(0, places.length - 2) : places;
        } catch (error) {
            return '';
        }
    },
    getProductCategories: () => {
        try {
            return Evergage.resolvers.buildCategoryId(".breadcrumbs a", 1, true, (categoryId) => [categoryId.toUpperCase()])
        } catch (error) {
            return null;
        }
    },
    getCategories: () => {
        try {
            const ignoreLast = audleySitemapConfigHelpers.getPageTemplateFromDataLayer() !== "country"
            return Evergage.resolvers.buildCategoryId(".breadcrumbs a", 1, ignoreLast, (categoryId) => categoryId.toUpperCase())
        } catch (error) {
            return null;
        }
    },
    isViewCategoryPageType: () => {
        try {
            return audleySitemapConfigHelpers.viewCategoryPageTypes.includes(audleySitemapConfigHelpers.getPageTemplateFromDataLayer());
        } catch (error) {
            return false;
        }
    },
    pageTypes: [
        {
            pageType: "itinerary",
            dataLayerTemplateName: "itinerary",
            sendProductData: true,
        },
        {
            pageType: "hb-itinerary",
            dataLayerTemplateName: "hb-itinerary",
            sendProductData: true,
        },
    ],
    viewCategoryPageTypes: ['country', 'itinerary-ideas', 'best-time-to-visit', 'places-to-go', 'country-guides']
};
Evergage.init({
    cookieDomain: "audleytravel.com",
}).then(() => {
    let favouriteItems = window.localStorage.getItem("audley-favourites");
    const sitemapConfig = {
        global: {
            onActionEvent: (actionEvent) => {
                actionEvent.user = actionEvent.user || {};
                actionEvent.user.attributes = actionEvent.user.attributes || {};
                const keys = audleySitemapConfigHelpers.getSfmcKeyFromURL();
                
                // Strict 1:1 dual key support mapping
                if (keys.newKey) {
                    actionEvent.user.attributes.tcesfmcContactKey = keys.newKey;
                }
                
                if (keys.oldKey) {
                    actionEvent.user.attributes.sfmcContactKey = keys.oldKey;
                }
                const page_template = audleySitemapConfigHelpers.getPageTemplateFromDataLayer();
                if (page_template === "itinerary-ideas" || page_template === "country" || page_template === "best-time-to-visit" 
                || page_template === "place" || page_template === "guide" || page_template === "superhotel"
                || page_template === "hotel" || page_template === "places-to-go" || page_template === "excursion"
                || page_template === "accommodation" || page_template === "things-to-do" || page_template === "super-region"
                || page_template === "country-guides" || page_template === "practical-info" || page_template === "ship"
                || page_template === "cruises"|| page_template === "country-guide") {
                    actionEvent.user.attributes.lastViewedDestination = audleySitemapConfigHelpers.getDestination();
                    actionEvent.user.attributes.lastViewedDestinationDate = audleySitemapConfigHelpers.getCurrentUTCDate();
                }

                if (page_template === "inspiration-hub"){
                    actionEvent.user.attributes.lastViewedHolidayType = audleySitemapConfigHelpers.getInspirationCategory();
                    actionEvent.user.attributes.lastViewedHolidayTypeDate = audleySitemapConfigHelpers.getCurrentUTCDate();
                }

                if (page_template === "contact-us") {
                    const formData = audleySitemapConfigHelpers.getContactFormInputData();
                    if (!actionEvent.user.attributes.lastEnquiryDestination) {
                        console.log('Make an Enquiry');
                        actionEvent.action = "Make an Enquiry"
                        actionEvent.user.attributes.enquiryStartDestination = formData.destination;
                        actionEvent.user.attributes.enquiryStartDate = audleySitemapConfigHelpers.getCurrentUTCDate();
                    }
                }
                if (page_template === "brochures") {
                    actionEvent.action = "Start a Brochure Request"
                }

                return actionEvent;
            },
            listeners: [
                Evergage.listener("click", ".InfinityNumber", () => {
                    Evergage.sendEvent({
                        action: "Unique Phone Number"
                    });
                }),
                Evergage.listener("change", "#Destination", () => {
                    const page_template = audleySitemapConfigHelpers.getPageTemplateFromDataLayer();
                    if (page_template === "contact-us") {
                        const formData = audleySitemapConfigHelpers.getContactFormInputData();
                        const user = {
                            attributes: {
                                enquiryStartDestination: formData.destination
                            }
                        };
                        Evergage.sendEvent({
                            user: user
                ...
                Evergage.listener("submit", 'form[action="/contact-us"]', () => {
                    const formData = audleySitemapConfigHelpers.getContactFormInputData();
                    if (formData && formData.email) {
                        let action = "Send Enquiry";
                        const user = {
                            attributes: {
                                ...
                                lastEnquiryDestination: formData.destination,
                                enquirySentDate: audleySitemapConfigHelpers.getCurrentUTCDate(),
                            }
                        };
                        Evergage.sendEvent({
                            action: action,
                ...
                Evergage.listener("submit", 'form[action="/us/contact-us"]', () => {
                    const formData = audleySitemapConfigHelpers.getContactFormInputData();
                    if (formData && formData.email) {
                        let action = "Send Enquiry";
                        const user = {
                ...
                Evergage.listener("submit", 'form[action="/brochures"]', () => {
                    const formData = audleySitemapConfigHelpers.getContactFormInputData();
                    if (formData && formData.email) {
                        let action = "Request a Brochure";
                        const user = {
                            attributes: {
                                emailAddress: formData.email,
                                firstName: formData.firstName,
                ...
                Evergage.listener("submit", 'form[action="/us/brochures"]', () => {
                    const formData = audleySitemapConfigHelpers.getContactFormInputData();
                    if (formData && formData.email) {
                        let action = "Request a Brochure";
                        const user = {
                ...
                Evergage.listener("click", ".favourite-btn", () => {
                    if (favouriteItems) {
                        let itemID = audleySitemapConfigHelpers.getItemID();
                        let exists = JSON.parse(favouriteItems).find(i => i.replaceAll("{", "").replaceAll("}", "") === itemID)
                        if (!exists)
                            Evergage.sendEvent({
                    ...
                    favouriteItems = window.localStorage.getItem("audley-favourites");
                }),
                Evergage.listener("submit", ".js-newsletter", () => {
                    const first_name = Evergage.cashDom('.input[name = "SignupFirstName"]').val();
                    const first_name_popup = Evergage.cashDom('.input[name = "popupFirst"]').val();
                    const first_name_magazine = Evergage.cashDom('.input[name = "magazineFirst"]').val();
                    const last_name = Evergage.cashDom('.input[name = "SignupSurname"]').val();
                    const last_name_popup = Evergage.cashDom('.input[name = "popupLast"]').val();
                    const last_name_magazine = Evergage.cashDom('.input[name = "magazineLast"]').val();
                    const email = Evergage.cashDom('.input[name = "SignupEmail"]').val();
                    const email_popup = Evergage.cashDom('.input[name = "popupEmail"]').val();
                    const email_magazine = Evergage.cashDom('.input[name = "magazineEmail"]').val();
                    if (email || email_popup || email_magazine) {
                        Evergage.sendEvent({
                            action: "Newsletter",
                            user: {
                                attributes: {
                                    emailAddress: email || email_popup || email_magazine,
                                    firstName: first_name || first_name_popup || first_name_magazine,
                                    lastName: last_name || last_name_popup || last_name_magazine,
                                }
                            }
                        });
                    }
                }), Evergage.listener("submit", '.js-newsletter[data-signup-type="popup"]', () => {
                    const first_name = Evergage.cashDom('.input[name = "SignupFirstName"]').val();
                    const first_name_popup = Evergage.cashDom('.input[name = "popupFirst"]').val();
                    const last_name = Evergage.cashDom('.input[name = "SignupSurname"]').val();
                    const last_name_popup = Evergage.cashDom('.input[name = "popupLast"]').val();
                    const email = Evergage.cashDom('.input[name = "SignupEmail"]').val();
                    const email_popup = Evergage.cashDom('.input[name = "popupEmail"]').val();
                    if (email || email_popup) {
                        Evergage.sendEvent({
                            action: "Newsletter",
                            user: {
                                attributes: {
                                    emailAddress: email || email_popup,
                                    firstName: first_name || first_name_popup,
                                    lastName: last_name || last_name_popup,
                                }
                            }
        ...
        pageTypes: [{
            ...
            isMatch: () => {
                return audleySitemapConfigHelpers.isViewCategoryPageType();
            },
            catalog: {
                Category: {
                    _id: () => audleySitemapConfigHelpers.getCategories(),
                },
            }
        },
        {
            name: "itinerary",
            isMatch: () => audleySitemapConfigHelpers.getPageTemplateFromDataLayer() === "itinerary",
            onActionEvent: (actionEvent) => {
                actionEvent.user = actionEvent.user || {};
                actionEvent.user.attributes = actionEvent.user.attributes || {};
                actionEvent.user.attributes.lastViewedItinerary = audleySitemapConfigHelpers.getPageName();
                actionEvent.user.attributes.lastViewedItineraryDate = audleySitemapConfigHelpers.getCurrentUTCDate();
                return actionEvent;
            },
            catalog: {
                Product: {
                    _id: audleySitemapConfigHelpers.getItemID(),
                    name: audleySitemapConfigHelpers.getPageName(),
                    categories: audleySitemapConfigHelpers.getProductCategories(),
                    url: Evergage.cashDom('head link[rel="canonical"]').attr('href'),
                    imageUrl: Evergage.cashDom('head meta[property="og:image"]').attr('content'),
                    price: audleySitemapConfigHelpers.getPrice(),
                    priceDescription: audleySitemapConfigHelpers.getPrice(),
                    currency: audleySitemapConfigHelpers.getCurrencyCode(),
                    tripDuration: audleySitemapConfigHelpers.getTripDuration(),
                    tourHighlights: audleySitemapConfigHelpers.getTourHighlights(),
                    placesVisited: audleySitemapConfigHelpers.getTourPlaces(),
                    relatedCatalogObjects: {
                        Destination: [audleySitemapConfigHelpers.getDestination()],
                        HolidayTypes: audleySitemapConfigHelpers.getHolidayTypes(),
                    },
                },
            }
        },
        {
            name: "hb-itinerary",
            isMatch: () => audleySitemapConfigHelpers.getPageTemplateFromDataLayer() === "hb-itinerary",
            onActionEvent: (actionEvent) => {
                actionEvent.user = actionEvent.user || {};
                actionEvent.user.attributes = actionEvent.user.attributes || {};
                actionEvent.user.attributes.lastViewedItinerary = audleySitemapConfigHelpers.getPageName();
